...
namespace Oxite.Modules.Blogs.Extensions
{
    ...
    {
        public static Post Apply(this Post post, PostInput input, EntityState state, User creator, Func<string, string> normalizeTag)
        {
            ...
            return new Post(post.Blog, input.Body, input.BodyShort, input.CommentingDisabled, post.Created, creator, post.ID, post.Modified, input.Published, input.Slug, state, tags, input.Title, post.Comments, post.Trackbacks);
        }
