...
namespace Oxite.Tests.Filters
{
    public class RssResultActionFilterTests
    {
            ...
            ActionExecutedContext context = new ActionExecutedContext()
                {
                    ...
                    Controller = new FakeController() { ViewData = new ViewDataDictionary(new OxiteViewModelItems<Post>() { Items = new List<Post>(new[] { new Post() }) }) }
                };
        ...
        public void OnExecutedSetsClientCachedToTrueForEmptyList()
        {
            ...
            {
                Result = new ViewResult(),
                Controller = new FakeController() { ViewData = new ViewDataDictionary(new OxiteViewModelItems<Post>() { Items = Enumerable.Empty<Post>().ToList() }) }
            };
