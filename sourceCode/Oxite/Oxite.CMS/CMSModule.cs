...
{
    public class CMSModule : IOxiteModule, IOxiteDataProvider
    {
        ...
        public void RegisterRoutes(RouteCollection routes)
        {
            routes.MapRoute(
                "Pages",
                "Admin/Pages",
                new { controller = "Page", action = "List", role = "Admin" },
                null,
        ...
        public void RegisterCatchAllRoutes(RouteCollection routes)
        {
            ...
            routes.MapRoute(
                "PageAddContent",
                "Admin/{*pagePath}",
                new { controller = "Page", action = "ItemAddContent", role = "Admin", validateAntiForgeryToken = true },
                new { pagePath = new IsPageMode(PageMode.AddContent) },
                controllerNamespaces
                );
        ...
        public void ConfigureProvider(OxiteConfigurationSection config, OxiteDataProviderConfigurationElement dataProviderConfig, IUnityContainer container)
        {
            if (dataProviderConfig.Category == "LinqToSql")
                container
                    .RegisterType<OxiteCMSDataContext>(new InjectionConstructor(new ResolvedParameter<string>(!string.IsNullOrEmpty(dataProviderConfig.ConnectionString) ? dataProviderConfig.ConnectionString : config.Providers.DefaultConnectionString)))
