...
namespace Oxite.Modules.CMS.Routing
{
    public class IsPageMode : IRouteConstraint
    {
        ...
        public bool Match(HttpContextBase httpContext, Route route, string parameterName, RouteValueDictionary values, RouteDirection routeDirection)
        {
            ...
            if (mode == PageMode.AddContent && pagePath.EndsWith("/" + PageMode.AddContent, StringComparison.OrdinalIgnoreCase))
                return true;

            if (mode == PageMode.EditContent && pagePath.EndsWith("/" + PageMode.EditContent, StringComparison.OrdinalIgnoreCase))
                return true;
