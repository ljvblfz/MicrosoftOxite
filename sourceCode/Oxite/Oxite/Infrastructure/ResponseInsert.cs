...
namespace Oxite.Infrastructure
{
    public class ResponseInsert
    {
        ...
        private static readonly Dictionary<string, Regex> compiledSelectorRegexes = new Dictionary<string, Regex>();
        private static readonly Regex whitespaceRegex = new Regex(@"\s+", RegexOptions.Singleline);
        private static readonly Regex compactSelectorRegex = new Regex(@"\s*([>+~,=!^$():\[\]])\s*", RegexOptions.Singleline);
        //todo: (nheskew) add filters and attributes
        private static readonly Regex simpleSelectorRegex = new Regex(@"^(?<tag>\w+)?(?:(?:\.(?<class>[a-z][\w-_]*))|(?:#(?<id>[a-z][\w-_.:]*)))*$", RegexOptions.Singleline);
        private static readonly Regex voidElementNameRegex = new Regex("^(?:br|img|input)$", RegexOptions.IgnoreCase);
        private static Func<string, IEnumerable<string>> generateSelectorFunction(string cssSelector)
        {
            if (!compiledSelectorRegexes.ContainsKey(cssSelector))
            {
            ...
            return doc => findElements(doc, compiledSelectorRegexes[cssSelector]);
        }
