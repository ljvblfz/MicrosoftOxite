<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<OxiteViewModelItems<ScheduleItemComment>>" %>
...
{ %><ul class="comments medium"><%
    int counter = 0;
    foreach (ScheduleItemComment comment in Model.Items)
    {
    ...
    <li class="<%=className.ToString() %>">
        <% Html.RenderPartialFromSkin("Comment", new OxiteViewModelPartial<ScheduleItemComment>(Model, comment)); %>
    </li><%
        counter++;
    } %>
</ul><% 
