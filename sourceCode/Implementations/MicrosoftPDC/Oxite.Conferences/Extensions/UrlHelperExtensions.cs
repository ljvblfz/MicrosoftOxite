...
namespace Oxite.Modules.Conferences.Extensions
{
    public static class UrlHelperExtensions
    {
        ...
        public static string ManageExhibitors(this UrlHelper urlHelper)
        {
            return urlHelper.RouteUrl("ManageExhibitors");
        }

        public static string EditExhibitor(this UrlHelper urlHelper, string name)
        {
            var exhibitorSlug = name.CleanSlug();

            var url = urlHelper.RouteUrl("EditExhibitor", new { exhibitorSlug });

            return url;
        }

        public static string AddExhibitor(this UrlHelper urlHelper)
        {
            var url = urlHelper.RouteUrl("AddExhibitor");
            
            return url;
        }

        public static string RemoveExhibitor(this UrlHelper urlHelper, string name)
        {
            var exhibitorSlug = name.CleanSlug();

            var url = urlHelper.RouteUrl("RemoveExhibitor", new { exhibitorSlug });
