...
namespace Oxite.Modules.Conferences.Models
{
    public class ScheduleItem : EntityBase, ITaggedEntity, ICommentedEntity
    {
        ...
        public ScheduleItem(Event e, Guid id, string title, string body, string location, string code, string type, DateTime start, DateTime end, string slug, IEnumerable<Speaker> speakers, IEnumerable<ScheduleItemTag> tags, IEnumerable<ScheduleItemComment> comments, IEnumerable<ScheduleItemUser> users, DateTime created, DateTime modified, IEnumerable<File> files)
            : this(id)
        {
            Event = e;
            Title = title;
