...
namespace Oxite.Controllers
{
    public class PostController : Controller
    {
        ...
        public virtual OxiteModelList<Post> ListByAreaAndTag(int? pageNumber, int pageSize, Area areaAddress, Tag tagAddress, DateTime? ifModifiedSince)
        {
            int pageIndex = pageNumber.HasValue ? pageNumber.Value - 1 : 0;

            Area area = areaService.GetArea(areaAddress.Name);
            if (area == null) return null;

            Tag tag = tagService.GetTag(tagAddress.Name);
            if (tag == null) return null;

            return GetPostList(area, () => postService.GetPosts(pageIndex, pageSize, area, tag, ifModifiedSince));
        }
        ...
        public virtual object SaveAdd(Area areaInput, Post postInput, User currentUser)
        {
            ...
            postInput.Area = areaService.GetArea(areaInput.Name);

            postService.AddPost(postInput, currentUser, out validationState, out newPost);
