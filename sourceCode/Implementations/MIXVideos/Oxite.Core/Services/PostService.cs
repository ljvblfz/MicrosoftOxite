...
namespace Oxite.Services
{
    public class PostService : IPostService
    {
        ...
        public IPageOfList<Post> GetPosts(int pageIndex, int pageSize, Area area, Tag tag, DateTime? sinceDate)
        {
            return sinceDate.HasValue ? repository.GetPosts(area, tag, sinceDate.Value.ToUniversalTime()).GetPage(pageIndex, pageSize) :  repository.GetPosts(area, tag).GetPage(pageIndex, pageSize);
        }
        ...
        public IPageOfList<Post> GetPostsWithDrafts(int pageIndex, int pageSize, Area area)
        {
            return repository.GetPostsWithDrafts(area).GetPage(pageIndex, pageSize);
        }

        public IPageOfList<Post> GetPostsByFileType(int pageIndex, int pageSize, string typeName)
        {
            return repository.GetPostsByFileType(typeName).GetPage(pageIndex, pageSize);
        }
        ...
        {
            IQueryable<Post> posts = repository.GetPosts(false);
        ...
        private string generateMessageBody(Post post, Comment comment, Site site)
        {
            ...
            body = body.Replace("{User.Name}", comment.Creator.GetDisplayName());
