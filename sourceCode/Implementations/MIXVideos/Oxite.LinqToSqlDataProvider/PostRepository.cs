...
namespace Oxite.LinqToSqlDataProvider
{
    public class PostRepository : IPostRepository
    {
        ...
        {
            return projectPosts(getPostsQuery(area));
        }
        ...
        public IQueryable<Post> GetPosts(Area area, Tag tag)
        {
            return projectPosts(excludeNotYetPublished(getPostsQuery(area, tag)));
        }

        public IQueryable<Post> GetPosts(Area area, Tag tag, DateTime sinceDate)
        {
            return projectPosts(excludeNotYetPublished(getPostsQuery(area, tag).Where(p => p.PublishedDate > sinceDate)));
        }

        private IQueryable<oxite_Post> getPostsQuery(Area area, Tag tag)
        {
            return
                from p in context.oxite_Posts
                join par in context.oxite_PostAreaRelationships on p.PostID equals par.PostID
                join ptr in context.oxite_PostTagRelationships on p.PostID equals ptr.PostID
                where par.AreaID == area.ID && ptr.TagID == tag.ID
                select p;
        }
        ...
        public IQueryable<Post> GetPostsByFileType(string typeName)
        {
            var query =
                ...
                where a.SiteID == siteID && p.PublishedDate <= DateTime.UtcNow && p.State == (byte)EntityState.Normal && string.Compare(f.TypeName, typeName, true) == 0
        ...
        private IQueryable<oxite_Post> getPostsQuery(SearchCriteria criteria)
        {
            return
                ...
                where a.SiteID == siteID && (p.Title.Contains(criteria.Term) || p.Body.Contains(criteria.Term) || p.Slug.Contains(criteria.Term))
                select p;
        }
            ...
            using (TransactionScope scope = new TransactionScope())
            {
                ...
                if (post.Files != null)
                {
                    foreach (File file in post.Files)
                    {
                        ...
                        {
                            persistenceFile = new oxite_File() { PostID = postToSave.PostID, Url = file.Url.ToString(), FileID = Guid.NewGuid() };
                            context.oxite_Files.InsertOnSubmit(persistenceFile);
                        }

                        persistenceFile.TypeName = file.TypeName;
        ...
        public Post GetPost(string areaName, string slug)
        {
            IQueryable<oxite_Post> post =
                ...
                where a.SiteID == siteID && a.AreaName == areaName && string.Compare(p.Slug, slug, true) == 0
                select p;
        ...
        private IQueryable<Post> projectPosts(IQueryable<oxite_Post> posts)
        {
            return from p in posts
                   ...
                   select projectPost(p, u, a, c, f, t, tb);
        }
        ...
        {
            return from f in context.oxite_Files
        ...
        private IQueryable<ParentAndChild<PostBase,Comment>> projectComments(IQueryable<oxite_Comment> comments)
        {
            return from c in comments
                   ...
                   {
                       Child = getComment(c, user),
                       Parent = projectPost(p, u, a, Enumerable.Empty<Comment>().AsQueryable(), Enumerable.Empty<File>().AsQueryable(), Enumerable.Empty<Tag>().AsQueryable(), Enumerable.Empty<Trackback>().AsQueryable())
                   };
        }
