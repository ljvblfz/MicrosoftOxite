...
<%@ Import Namespace="Oxite.Models.Extensions" %>
    <form method="post" id="comment" action="<%=Url.AddCommentToPost(Model.Item) %>#comment"><%
        ...
        else if (Request.UrlReferrer != null && ViewData.ModelState.IsValid && Request.Url.AbsoluteUri == Request.UrlReferrer.AbsoluteUri)
        { %>
        <div class="message info"><%=Model.Localize("SlowComment", "Thanks for the comment. It'll show up here shortly if it hasn't already made it through.")%></div><%
        } %>
        <fieldset class="info">
            ...
            <div class="email">
                ...
                <%=Html.TextBox("email", (Request.Form["email"] ?? (Model.User != null ? Model.User.Email : "")), new { id = "comment_email", @class = "text", tabindex = "2", title = Model.Localize("comment_email","Your email...") }) %><%= Html.ValidationMessage("PostSubscription.Email", "Valid email required to subscribe.") %><%= Html.ValidationMessage("UserBase.Email", "Invalid email.") %>
            ...
            <div class="url">
                ...
                <%=Html.TextBox("url", (Request.Form["url"] ?? (Model.User != null ? Model.User.Url : "")), new { id = "comment_url", @class = "text", tabindex = "3", title = Model.Localize("comment_url", "Your home on the interwebs (URL)...") })%><%= Html.ValidationMessage("UserBase.Url", "URL looks a little off.") %>
            </div>
            <div class="remember">
