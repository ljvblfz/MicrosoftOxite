    ...
    public class SqlServerPostRepository : IPostRepository
    {
        ...
        public Post Save(Post post)
        {
            ...
            foreach (Tag tag in post.Tags)
            {
                ...
                if (!context.oxite_Blogs_PostTagRelationships.Any(pt => pt.PostID == postToSave.PostID && pt.TagID == persistenceTag.TagID))
                    context.oxite_Blogs_PostTagRelationships.InsertOnSubmit(new oxite_Blogs_PostTagRelationship { PostID = postToSave.PostID, TagID = persistenceTag.TagID });
            }
        ...
        public PostTag Save(Post post, PostTag tag)
        {
            ...
            return (
                ...
                select new PostTag(t.TagID, t.TagName, t.TagName, t.CreatedDate)
                ).FirstOrDefault();
        }
        ...
        private static Post projectPost(oxite_Blogs_Post p, oxite_User u, oxite_Blogs_Blog b, IQueryable<PostComment> c, IQueryable<oxite_Blogs_PostTagRelationship> t, IQueryable<Trackback> tb, IQueryable<File> f)
        {
            ...
            return new Post(blog, p.Body, p.BodyShort, p.CommentingDisabled, p.CreatedDate, creator, p.PostID, p.ModifiedDate, p.PublishedDate, p.Slug, (EntityState)p.State, t.Select(tag => new PostTag(tag.TagID, tag.oxite_Tag.TagName)).ToList(), p.Title, c.ToList(), tb.ToList(), f.ToArray());
        }
