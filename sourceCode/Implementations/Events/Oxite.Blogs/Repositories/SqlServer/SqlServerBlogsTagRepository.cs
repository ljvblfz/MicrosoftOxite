    ...
    public class SqlServerTagRepository : IBlogsTagRepository
    {
        ...
        public IQueryable<KeyValuePair<PostTag, int>> GetTagsWithPostCount()
        {
            return from tt in
                   ...
                   group tt by new { Tag = tt.Tag } into results
                   where results.Key.Tag.TagID == results.Key.Tag.ParentTagID
                   orderby results.Key.Tag.TagName
                   select new KeyValuePair<PostTag, int>(new PostTag(results.Key.Tag.TagID, results.Key.Tag.TagName, results.Key.Tag.TagName, results.Key.Tag.CreatedDate), results.Count());
        }
