	...
	{ %>
	    <%=string.Format(
	        Model.Localize("SessionBrowser.CurrentTagFormat", "Tag: {0}"), 
	            string.Format("<span class=\"current tag\">{0}</span>", scheduleItemTag.DisplayName))%>&nbsp;&nbsp;&nbsp;<%=Html.Link(Model.Localize("Filter.Remove", "&nbsp; &nbsp;"), Url.Sessions(new ScheduleItemFilterCriteria{ Term = Request["term"] }), new { @class = "closebutton", title = Model.Localize("Filter.Clear", "clear") }) %><%
	}    %></div>
</div>
