...
%>
	<div class="<%=statusClass%>">
		<h3><%=
        string.Format(
            Model.Item.Comments.Count() == 1
                ? Model.Localize("SingleCommentStatus", "{0} Comment")
                : Model.Localize("MultiCommentStatus", "{0} Comments"), Model.Item.Comments.Count())%></h3>
		<%
            if (Model.Item.Comments.Count() > 3)
            {%>
