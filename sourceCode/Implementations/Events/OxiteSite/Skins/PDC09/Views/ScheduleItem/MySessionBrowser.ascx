...
<div class="controlbar">
   ...
   if (scheduleItemTag != null)
	{ %>
	    <%=string.Format(
	        Model.Localize("SessionBrowser.CurrentTagFormat", "Tag: {0}"), 
	            string.Format("<span class=\"current tag\">{0}</span>", scheduleItemTag.DisplayName))%>&nbsp;<%=Html.Link(Model.Localize("Filter.Remove", "remove filter"), Url.Sessions(new ScheduleItemFilterCriteria{ Term = Request["term"] }), new { @class = "closebutton", title = Model.Localize("Filter.Clear", "clear") }) %><%
	}    %></div>
</div>
...
<div id="sessiontags">
	...
	<%=Html.UnorderedList(
        tagListViewModel.Tags.OrderBy(t => t.DisplayName).Columns(3),
                (t, i) => Html.Link(t.Tag.DisplayName.Ellipsize(21, s => s.CleanText()), Url.MySessions(t.Tag)), null,
