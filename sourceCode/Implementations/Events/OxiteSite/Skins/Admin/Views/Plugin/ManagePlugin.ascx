...
if (Model.PartialModel.Container.CompilationException == null)
{
    ...
    %><form class="disablePlugin modifyPlugin" method="POST" action="<%=Url.PluginDisable(Model.PartialModel) %>">
        <input type="image" src="<%=Url.CssPath("images/plugin_disabled.png" alt="<%=Model.Localize("Disable") %>" title="<%=Model.Localize("Disable") %>" class="ibutton image pluginDisable" />
    ...
    %><form class="enablePlugin modifyPlugin" method="POST" action="<%=Url.PluginEnable(Model.PartialModel) %>">
        <input type="image" src="<%=Url.CssPath("images/plugin.png" alt="<%=Model.Localize("Enable") %>" title="<%=Model.Localize("Enable") %>" class="ibutton image pluginEnable" />
        <input type="hidden" name="returnUrl" value="<%=returnUrl %>">
        <%=Html.OxiteAntiForgeryToken() %>
    </form><%
    }
