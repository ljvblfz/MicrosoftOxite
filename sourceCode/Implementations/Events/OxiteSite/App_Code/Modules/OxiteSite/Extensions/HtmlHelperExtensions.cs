...
namespace OxiteSite.App_Code.Modules.OxiteSite.Extensions
{
    public static class HtmlHelperExtensions
    {
        ...
        public static void RenderPlayer<TModel>(this HtmlHelper<TModel> htmlHelper,
                                                string viewName,
                                                IEnumerable<File> files,
                                                string bug,
                                                string canonicalURL) where TModel : OxiteViewModel
        {
            htmlHelper.RenderPlayer(viewName, files, bug, canonicalURL, "","");
        }
        ...
        public static void RenderPlayer<TModel>(this HtmlHelper<TModel> htmlHelper, 
                                                ...
                                                string canonicalURL,
                                                string defaultPath,
                                                string streamingPath) where TModel : OxiteViewModel
        {
            ...
            if (fileList.Count() > 0)
            {
                ...
                if (media != null)
                {
                    ...
                    switch (media.TypeName)
                    {
                        ...
                        case "WMVHigh":
                            currentStream = "High Quality WMV (960x540)";
                            break;
                    ...
                    if (media.TypeName == "Smooth")
                    {
                      if (wmv != null || wmvHigh != null)
                      {
                          ...
                          if (wmvHigh != null)
                          {
                              ...
                              options += string.Format("the <a href=\"{0}\">High Quality WMV (960x540, not smooth streaming)</a> video ", canonicalURL + "?type=wmvhigh");
                          }
                      }
                    ...
                    htmlHelper.RenderPartialFromSkin(viewName,
                                                     new OxiteViewModelPartial<PlayerViewModel>(
                                                         htmlHelper.ViewData.Model, new PlayerViewModel(media, preview, message, bug, defaultPath, streamingPath)),
                                                     htmlHelper.ViewData);
