...
{ %><ul class="comments small"><%
    ...
    foreach (PostComment postAndComment in Model.Items)
    {
        ...
        if (Model.User.IsInRole("Admin")) { className.AppendFormat(" {0}", postAndComment.State.ToString().ToLower()); }
    ...
    <li class="<%=className.ToString() %>">
            <div class="meta" id="<%=postAndComment.Slug %>"><%
                if (Model.User.IsInRole("Admin") && postAndComment.State == EntityState.PendingApproval)
                {
                ...
                <span class="when"> - <%= 
                    Html.Link(
                        ...
                        Model.User.IsInRole("Admin") && postAndComment.State == EntityState.PendingApproval
