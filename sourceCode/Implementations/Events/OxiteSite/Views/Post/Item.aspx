<%@ Page Language="C#" AutoEventWireup="true" Inherits="System.Web.Mvc.ViewPage<OxiteViewModelItem<Post>>" %>
...
<div class="post">
    ...
    <div class="message"><%=Model.Localize("CommentingDisabled", "Commenting is disabled for this post.")%></div><%
        ...
        <%= Html.WebBug("post", Model.Item.ID.ToString("N")) %>
</div>
</asp:Content>
...
<asp:Content ContentPlaceHolderID="MetaDescription" runat="server">
    <%=Html.PageDescription(Model.Item.GetBodyShort()) %><%=Html.MetaTag("Keywords", string.Join(", ", Model.Item.Tags.Select(s => s.DisplayName).ToArray()), true) %>
</asp:Content>
