...
<%@ Import Namespace="Oxite.Modules.Blogs.Models" %>
  ...
  <updated><%=XmlConvert.ToString(Model.Items.First().Published.Value, XmlDateTimeSerializationMode.Utc) %></updated><%
  ...
  <generator uri="<%=Url.Oxite() %>" version="1.0">Oxite</generator><%
    ...
    {
        Html.RenderPartialFromSkin("AtomFeedEntry", new OxiteViewModelPartial<Post>(Model, post));
    } %>