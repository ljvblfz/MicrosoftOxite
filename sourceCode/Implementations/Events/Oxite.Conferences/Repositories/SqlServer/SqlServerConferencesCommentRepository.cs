...
namespace Oxite.Modules.Conferences.Repositories.SqlServer
{
    public class SqlServerConferencesCommentRepository : IConferencesCommentRepository
    {
        ...
        public ScheduleItemCommentShell GetComment(Guid siteID, string eventName, string scheduleItemSlug, string commentSlug)
        {
            return (
        ...
        public ScheduleItemComment Save(ScheduleItemComment comment, Guid siteID, string eventName, string scheduleItemSlug)
        {
            ...
            oxite_Conferences_ScheduleItem scheduleItem = (from si in context.oxite_Conferences_ScheduleItems join e in context.oxite_Conferences_Events on si.EventID equals e.EventID where /*e.SiteID == siteID && */string.Compare(e.EventName, eventName, true) == 0 && string.Compare(si.Slug, scheduleItemSlug, true) == 0 select si).FirstOrDefault();
            if (scheduleItem == null) throw new InvalidOperationException(string.Format("ScheduleItem in Event {0} at slug {1} could not be found to add the comment to", eventName, scheduleItemSlug));
            ...
            return (
                ...
                where /*e.SiteID == siteID && */string.Compare(e.EventName, eventName, true) == 0 && string.Compare(si.Slug, scheduleItemSlug, true) == 0 && string.Compare(sicr.Slug, comment.Slug, true) == 0
