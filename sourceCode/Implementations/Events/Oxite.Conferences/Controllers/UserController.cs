    ...
    public class UserController : Controller
    {
        ...
        public ActionResult Token()
        {
            ...
            if (user != null && user.IsAuthenticated)
            {
                ...
                var token = userService.GetModuleData(user.Name, "APIKey");
                if(string.IsNullOrEmpty(token))
                {
                    token = Guid.NewGuid().ToString().ToLowerInvariant().Replace("-", "");
                    userService.SetModuleData(user.Name, "APIKey", token);
                }
